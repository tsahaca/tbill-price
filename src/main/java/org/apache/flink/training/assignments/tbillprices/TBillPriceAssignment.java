...
public class TBillPriceAssignment extends ExerciseBase {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if( null != ExerciseBase.priceOut) {
            returns.addSink(ExerciseBase.priceOut);
        }
        ...
        // Logging Example 1
        /**
        final var returnsSink =
                returns.addSink(new SinkFunction<>() {
                    @Override
                    public void invoke(Tuple3<LocalDateTime, Double, Boolean> value) {
        ...
        **/
        ...
        //printOrTest(avgReturn);

        if( null != ExerciseBase.averageOut ) {
            avgReturn.addSink(ExerciseBase.averageOut);
        }
        ...
        // Logging: Example 2
        /**
        final var avgReturnSink =
                avgReturn.addSink(new LogSink<>(LOG, LogSink.LoggerEnum.ERROR, "**** Element {}"));
        */

        //DataStreamSink mySink = avgReturn.addSink(new LogSink<>(LOG, LogSink.LoggerEnum.ERROR, "**** Element {}"));
        ...
        //printOrTest(dailyReturns);

        //final DataStream<MonthlyAverageReturn> monthlyAverageReturns = avgReturn.map(item -> new MonthlyAverageReturn(item.f0.format(DateTimeFormatter.ofPattern("yyyy-MM")), new Float(item.f1)))
        ...
        //printOrTest(volatility);

        if( null != ExerciseBase.volatilityOut){
            volatility.addSink(ExerciseBase.volatilityOut);
        }
